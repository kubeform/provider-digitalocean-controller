...
// SpacesBucketReconciler reconciles a SpacesBucket object
type SpacesBucketReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=spacesbucket.digitalocean.kubeform.com,resources=spacesbuckets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=spacesbucket.digitalocean.kubeform.com,resources=spacesbuckets/status,verbs=get;update;patch

func (r *SpacesBucketReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("spacesbucket", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch SpacesBucket")
...
func (r *SpacesBucketReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &spacesbucketv1alpha1.SpacesBucket{}); err != nil {
			klog.Error(err, "unable to set up auditor", spacesbucketv1alpha1.SpacesBucket{}.APIVersion, spacesbucketv1alpha1.SpacesBucket{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&spacesbucketv1alpha1.SpacesBucket{}).
