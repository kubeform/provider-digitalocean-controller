...
// DockerCredentialsReconciler reconciles a DockerCredentials object
type DockerCredentialsReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=containerregistry.digitalocean.kubeform.com,resources=dockercredentials,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=containerregistry.digitalocean.kubeform.com,resources=dockercredentials/status,verbs=get;update;patch

func (r *DockerCredentialsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("dockercredentials", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch DockerCredentials")
...
func (r *DockerCredentialsReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerregistryv1alpha1.DockerCredentials{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerregistryv1alpha1.DockerCredentials{}.APIVersion, containerregistryv1alpha1.DockerCredentials{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerregistryv1alpha1.DockerCredentials{}).
...
func (r *DockerCredentialsReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for DockerCredentials")
			return nil
		}
		...
		resourceList := &containerregistryv1alpha1.DockerCredentialsList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list DockerCredentials")
			return nil
		}
