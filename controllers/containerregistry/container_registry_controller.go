...
// ContainerRegistryReconciler reconciles a ContainerRegistry object
type ContainerRegistryReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=containerregistry.digitalocean.kubeform.com,resources=containerregistries,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=containerregistry.digitalocean.kubeform.com,resources=containerregistries/status,verbs=get;update;patch

func (r *ContainerRegistryReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("containerregistry", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ContainerRegistry")
...
func (r *ContainerRegistryReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &containerregistryv1alpha1.ContainerRegistry{}); err != nil {
			klog.Error(err, "unable to set up auditor", containerregistryv1alpha1.ContainerRegistry{}.APIVersion, containerregistryv1alpha1.ContainerRegistry{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&containerregistryv1alpha1.ContainerRegistry{}).
