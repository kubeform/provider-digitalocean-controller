...
// +kubebuilder:rbac:groups=droplet.digitalocean.kubeform.com,resources=droplets,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=droplet.digitalocean.kubeform.com,resources=droplets/status,verbs=get;update;patch

func (r *DropletReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("droplet", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Droplet")
...
func (r *DropletReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dropletv1alpha1.Droplet{}); err != nil {
			klog.Error(err, "unable to set up auditor", dropletv1alpha1.Droplet{}.APIVersion, dropletv1alpha1.Droplet{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dropletv1alpha1.Droplet{}).
