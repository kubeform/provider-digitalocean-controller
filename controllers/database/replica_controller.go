...
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=replicas,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=replicas/status,verbs=get;update;patch

func (r *ReplicaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("replica", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Replica")
...
func (r *ReplicaReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.Replica{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.Replica{}.APIVersion, databasev1alpha1.Replica{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.Replica{}).
...
func (r *ReplicaReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Replica")
			return nil
		}
		...
		resourceList := &databasev1alpha1.ReplicaList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Replica")
			return nil
		}
