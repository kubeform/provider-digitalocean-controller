...
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=dbs,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=dbs/status,verbs=get;update;patch
...
func (r *DbReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Db")
...
func (r *DbReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.Db{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.Db{}.APIVersion, databasev1alpha1.Db{}.Kind)
			return err
		}
	}
