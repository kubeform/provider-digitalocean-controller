...
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=connectionpools,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=database.digitalocean.kubeform.com,resources=connectionpools/status,verbs=get;update;patch
...
func (r *ConnectionPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ConnectionPool")
...
func (r *ConnectionPoolReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &databasev1alpha1.ConnectionPool{}); err != nil {
			klog.Error(err, "unable to set up auditor", databasev1alpha1.ConnectionPool{}.APIVersion, databasev1alpha1.ConnectionPool{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&databasev1alpha1.ConnectionPool{}).
...
func (r *ConnectionPoolReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for ConnectionPool")
			return nil
		}
		...
		resourceList := &databasev1alpha1.ConnectionPoolList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ConnectionPool")
			return nil
		}
